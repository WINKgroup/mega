 ...
 */
import { getKeypress, question } from '@winkgroup/misc/build/node'
import fs from 'fs'
...
import { StorageMegaMethodResponse, StorageMegaTransferResult } from '../src/common'
            ...
            case '5':
                {
                    ...
                    const remotepath = await question('remotepath: ')
                    const storage = new StorageMega({ email: email, password: password })
...
s = stop                    
                    ...
                    eventEmitter.on('started', (info:{ totalBytes: number}) => {
                        bar.start(info.totalBytes, 0)
                    })

                    eventEmitter.on('progress', (info: {bytes: number}) => bar.update(info.bytes) )

                    const uploader = () => new Promise<StorageMegaMethodResponse<StorageMegaTransferResult>>( async resolve => {
                        let ended = false
                        storage.upload(localpath, remotepath, { onProgress: eventEmitter }).then( (result) => {
                            ended = true
                            resolve(result)
                        } )
                        while( !ended ) {
            ...
            case '7':
                {
                    const remotepath = await question('proxy to set (special: "none" | "auto"): ')
