...
export type StorageMegaLockAndLogin =
    | 'already locked'
    | 'newly locked'
    | 'unable to lock'
    | 'unable to login'
...
export class StorageMega {
    ...
    constructor(input: StorageMegaInput) {
        ...
        this.consoleLog = input.consoleLog
            ? input.consoleLog
            : new ConsoleLog({ prefix: 'StorageMega' });
    ...
    async df(
        ...
        if (!result)
            return { state: 'error', error: 'unable to df / parsing error' };
    ...
    async getPathType(
        ...
        if (result === false)
            return { state: 'error', error: 'unable to get path type' };
        return { state: 'success', result: result };
    }
    ...
    async isFileOk(
        ...
        if (byteDiff > options.toleranceBytesPercentage)
            return {
                ...
                result: {
                    isOk: false,
                    message: 'file size too different',
                    remoteBytes: file.bytes,
                },
            };
        return {
            state: 'success',
            result: { isOk: true, remoteBytes: file.bytes },
        };
    }
    ...
    async upload(
        ...
        const filesToUpload = options.transfers
            ? options.transfers
            : await megaCmd.put2transfers(localpath, remotepath);
        if (!filesToUpload)
            return setError('unable to determinate transfers during upload');
        ...
        if (!options.allowOverwrite) {
            if (filesToUpload.transfers.length > 1) {
                ...
                if (fileTypeResponse.state === 'error')
                    ...
                    error = `overwriting "${remotepath}" not allowed`;
        ...
        if (dfResult.result!.freeBytes <= filesToUpload.totalBytes)
            return setError('not enough space');
        ...
        for (const fileTransferExpected of filesToUpload.transfers) {
            ...
            if (options.deleteOriginal && isFileOkResult.isOk)
                fs.unlinkSync(fileTransferExpected.sourcePath);
        }
    ...
    async masterkey() {
        ...
        return masterkey
            ? { state: 'success', result: masterkey }
            : { state: 'error', error: 'parsing error' };
    }
